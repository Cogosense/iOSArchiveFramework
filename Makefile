...
Build_$(3) : $(MAKER_BUILDROOT_DIR)/$(3)$(INSTALLED_LIB)

$(MAKER_BUILDROOT_DIR)/$(3)$(INSTALLED_LIB) :
	$(MAKE) -C $(MAKER_BUILD_DIR)/$(1) DESTDIR=$(MAKER_BUILDROOT_DIR)/$(3) LIBTOOLFLAGS=--no-warn install
...
bundle-dirs :
	...
	mkdir -p $(BUILT_PRODUCTS_DIR)/$(FRAMEWORKBUNDLE)/Resources
	mkdir -p $(BUILT_PRODUCTS_DIR)/$(FRAMEWORKBUNDLE)/Headers
	mkdir -p $(BUILT_PRODUCTS_DIR)/$(FRAMEWORKBUNDLE)/Documentation
	mkdir -p $(BUILT_PRODUCTS_DIR)/$(FRAMEWORKBUNDLE)/Modules
